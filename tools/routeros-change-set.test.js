...
assert.throws(()=>createChangeSet(d,{id:'spoofed'}),/User-controlled Change Set IDs are forbidden/);
const rb=prepareRollback(cs);assert.strictEqual(rb.rollback.prepared,true);assert.strictEqual(rb.rollback.artifact.reversible,true);assert.strictEqual(rb.rollback.artifact.commands.length,2);assert.notStrictEqual(rb.fingerprint,cs.fingerprint);
