...
assert.throws(() => recordTargetOutcome(failed, targetIds[0], 'completed'), /Terminal operation target outcome is immutable/);
assert.throws(() => recordTargetOutcome(failed, targetIds[0], 'skipped'), /Terminal operation target outcome is immutable/);
const completed = recordTargetOutcome(plan, targetIds[0], 'completed');
assert.strictEqual(completed.targets.find(t => t.router_id === targetIds[0]).status, 'completed');
assert.throws(() => recordTargetOutcome(completed, targetIds[0], 'failed'), /Terminal operation target outcome is immutable/);
assert.strictEqual(validateFleetOperationPlan(completed, fleet).valid, true);
