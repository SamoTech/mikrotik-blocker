...
const { buildRemediation } = require('../tools/firewall-remediation');
...
module.exports = function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json(buildRemediation(config));
  } catch (error) {
    return res.status(400).json({ error: 'Unable to generate remediation', detail: error.message });
  }
};
